...
import NavItemCheck from '../NavItemCheck'
...
const NavCollapse = ({ menu, level, parentId }) => {
    ...
    const { disabled, drawerOpen } = useSelector((state) => state.menu);
    ...
    const menus = menu.children?.map((item) => {
        switch (item.type) {
            ...
            case 'checkItem':
                return <NavItemCheck isParents key={item.id} item={item} level={level + 1} parentId={parentId} />;
            case 'historyItem':
                return <NavHistoryItem isParents key={item.id} item={item} level={level + 1} parentId={parentId} />;
            case 'item':
